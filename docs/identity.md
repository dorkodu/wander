...
We use two interrelated forms of identifiers: the *handle* and the *DID*. Handles are DNS names while DIDs are an [emerging W3C standard](https://www.w3.org/TR/did-core/) which act as secure & stable IDs.
...
| **Handles**       | Handles are DNS names. They are resolved using the [com.atproto.identity.resolveHandle()](https://atproto.com/lexicons/com-atproto-identity) XRPC method and should be confirmed by a matching entry in the DID document. Details in the [Handle specification](https://atproto.com/specs/handle). |
| ----------------- | ------------------------------------------------------------ |
| **DIDs**          | DIDs are an emerging [W3C standard](https://www.w3.org/TR/did-core/) for providing stable & secure IDs. They are used as stable, canonical IDs of users. Details of how they are used in AT Protocol in the [DID specification](https://atproto.com/specs/did). |
| **DID Documents** | DID Documents are standardized objects which are hosted by DID registries. They include the following information:The handle associated with the DID.The signing key.The URL of the user’s PDS. |

## DID Methods[#](https://atproto.com/guides/identity#did-methods)
...
## Handle Resolution[#](https://atproto.com/guides/identity#handle-resolution)
...
Handle resolution uses the [`com.atproto.identity.resolveHandle`](https://atproto.com/lexicons/com-atproto-identity) XRPC method. The method call should be sent to the server identified by the handle, and the handle should be passed as a parameter.
...
### Example: Hosting service[#](https://atproto.com/guides/identity#example-hosting-service)
...
### Example: Hosting service with separate domain name[#](https://atproto.com/guides/identity#example-hosting-service-with-separate-domain-name)
...
### Example: Self-hosted[#](https://atproto.com/guides/identity#example-self-hosted)
